...
function restoreTheme() {
  // Your code here
  //   const themeName = document.cookie.split("=")[1];
  ...
  const themeName = cookies["themeName"];
...
const cookieParser = () => {
  ...
  // goal: "themeName=dragon; displayName='demo user'"   ====>  {themeName: 'dragon', displayName: 'demo user'}
  const cookieObj = {};

  cookies.split(";").forEach((element) => {
    // [themeName=dragon, displayName='demo user]
    let [key, value] = element.split("="); // themeName=dragon => [themeName, dragon]  = [themeName,dragon]
